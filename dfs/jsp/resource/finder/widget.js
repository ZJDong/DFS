...
this.seed++;return this.seed;};var StringUtil={};StringUtil.trim=function(s){return(s!=null?new String(s).replace(/(^\s*)|(\s*$)/g,""):"");};StringUtil.startsWith=function(source,search){if(source.length>=search.length){return(source.substring(0,search.length)==search);}
...
return buffer.join("");};var BindUtil={};BindUtil.bind=function(object,handler){return function(){return handler.apply(object,arguments);}};BindUtil.bindAsEventListener=function(object,handler){return function(event){return handler.call(object,(event||window.event));}};var EventUtil={};EventUtil.addEventListener=function(target,type,handler){if(target.addEventListener){target.addEventListener(type,handler,false);}
...
else{target["on"+type]=null;}};EventUtil.stop=function(event,returnValue){if(event!=null){if(event.stopPropagation){event.stopPropagation();}
else{event.cancelBubble=true;}
if(event.preventDefault){event.preventDefault();}
if(returnValue==true){event.cancel=false;event.returnValue=true;}
else{event.cancel=true;event.returnValue=false;}}
return false;};var Dragable=function(source,target){this.x=0;this.y=0;this.source=source;this.target=target;this.frame=document.getElementById("_dragable_frame");this.start=function(event){var src=(event.srcElement||event.target);var keyCode=(event.keyCode||event.which);if(keyCode!=1){return true;}
if(src.getAttribute("dragable")=="false"){return true;}
this.y=event.clientY-this.target.offsetTop;this.x=event.clientX-this.target.offsetLeft;var x=this.target.offsetLeft;var y=this.target.offsetTop;var w=this.target.offsetWidth;var h=this.target.offsetHeight;this.frame.style.top=y+"px";this.frame.style.left=x+"px";this.frame.style.width=w+"px";this.frame.style.height=h+"px";this.frame.style.display="block";this.frame.style.zIndex=parseInt(this.target.style.zIndex)+10;EventUtil.addEventListener(document,"mouseup",this.stopHandler);EventUtil.addEventListener(document,"mousemove",this.moveHandler);return true;};this.move=function(event){var x=event.clientX-this.x;var y=event.clientY-this.y;this.frame.style.top=y+"px";this.frame.style.left=x+"px";this.frame.style.zIndex=parseInt(this.target.style.zIndex)+10;return this.stopPropagation(event);};this.stop=function(event){var y=this.frame.offsetTop;var x=this.frame.offsetLeft;this.target.style.marginTop="0px";this.target.style.marginLeft="0px";this.target.style.top=y+"px";this.target.style.left=x+"px";this.frame.style.zIndex=-1;this.frame.style.display="none";EventUtil.removeEventListener(document,"mouseup",this.stopHandler);EventUtil.removeEventListener(document,"mousemove",this.moveHandler);return this.stopPropagation(event);};this.stopPropagation=function(event){if(event.stopPropagation){event.stopPropagation();}
else{event.cancelBubble=true;}
if(event.preventDefault){event.preventDefault();}
event.cancel=true;event.returnValue=false;return false;};this.stopHandler=BindUtil.bindAsEventListener(this,this.stop);this.moveHandler=BindUtil.bindAsEventListener(this,this.move);if(this.frame==null){this.frame=document.createElement("div");this.frame.id="_dragable_frame";if(document.all){this.frame.innerHTML="<div style=\"width: 100%; height: 100%; background-color: #0e89e6; filter: alpha(opacity=10); cursor: default;\"></div>";}
else{this.frame.innerHTML="<div style=\"width: 100%; height: 100%; background-color: #0e89e6; opacity: 0.1; cursor: default;\"></div>";}
document.body.appendChild(this.frame);}
if(this.target!=null){this.target.style.position="absolute";var y=this.target.offsetTop;var x=this.target.offsetLeft;var w=this.target.offsetWidth-0;var h=this.target.offsetHeight-0;var cssText="position: absolute; display: none;"
+" width: "+w+"px; height: "+h+"px; top:"+y+"px; left:"+x+"px;"
+" border: 1px dashed #3399FF; background-color: transparent; cursor: default; z-index: -1";this.frame.style.cssText=cssText;}
if(this.source!=null){EventUtil.addEventListener(this.source,"mousedown",BindUtil.bindAsEventListener(this,this.start));}};Dragable.registe=function(source,target){var e1=null;var e2=null;if(typeof(source)=="string"){e1=document.getElementById(source);}
else{e1=source;}
if(typeof(target)=="string"){e2=document.getElementById(target);}
else{e2=target;}
if(e1!=null&&e2!=null){new Dragable(e1,e2);}};var TaskManager={};TaskManager.id="_task_bar";TaskManager.tasks=new ArrayList();TaskManager.getById=function(id){var dialog=null;this.tasks.each(function(e){if(e.getId()==id){dialog=e;return false;}
else{return true;}});return dialog;};TaskManager.add=function(dialog){var dialogId=dialog.getId();if(this.getById(dialogId)==null){this.tasks.remove(dialog);this.tasks.add(dialog);this.show();}};TaskManager.remove=function(dialog){var task=this.getById(dialog.getId());this.tasks.remove(task);jQuery("#"+this.id+" ul li[dialogId="+dialog.getId()+"]").remove();};TaskManager.show=function(){var c=document.getElementById(this.id);if(c!=null){var buffer=[];var tasks=this.tasks;buffer[buffer.length]="<ul class=\"widget-task-bar\">";for(var i=0,size=tasks.size();i<size;i++){var dialog=tasks.get(i);if(dialog.parent==null){buffer[buffer.length]="<li dialogId=\""+dialog.getId()+"\" title=\""+dialog.title+"\"onclick=\"TaskManager.toggle(this)\">";if(dialog.icon!=null){buffer[buffer.length]="<img style=\"margin-top: -2px; margin-right: 4px;\" src=\""+dialog.icon+"\">";}
buffer[buffer.length]=dialog.title+"</li>";}}
buffer[buffer.length]="</ul>";c.innerHTML=buffer.join("");}};TaskManager.setTitle=function(dialog){var dialogId=dialog.getId();if(dialog.icon!=null){jQuery("#"+this.id+" ul li[dialogId="+dialogId+"]").html("<img style=\"margin-top: -2px; margin-right: 4px;\" src=\""+dialog.icon+"\">"+dialog.title);}
else{jQuery("#"+this.id+" ul li[dialogId="+dialogId+"]").html(dialog.title);}};TaskManager.toggle=function(src){var dialog=this.getById(src.getAttribute("dialogId"));if(dialog!=null){dialog.toggle();}};TaskManager.getContainer=function(){return document.getElementById(this.id);};var DialogManager={};DialogManager.zIndex=1000;DialogManager.active=null;DialogManager.dialogs=new ArrayList();DialogManager.setActive=function(dialog,b){if(b==true){this.push(dialog);}
else{this.pop(dialog);}};DialogManager.add=function(dialog){if(dialog.parent==null){var last=this.dialogs.last();if(last!=dialog){this.dialogs.remove(dialog);this.dialogs.add(dialog);}
DialogManager.show();}};DialogManager.push=function(dialog){var last=this.dialogs.last();if(last!=dialog){this.dialogs.remove(dialog);this.dialogs.add(dialog);this.dialogs.each(function(e){if(e!=dialog){e.setActiveStyle(false);}});dialog.setActiveStyle(true);}
dialog.focus();};DialogManager.pop=function(dialog){this.dialogs.remove(dialog);dialog.setActiveStyle(false);var dialog=this.getActive();if(dialog!=null){this.push(dialog);}
...
e.style.width=width+"px";e.style.height=height+"px";if(e.childNodes.length>0){var frame=e.childNodes[0];frame.style.width=width;frame.style.width=height;}}};Mask.close=function(id){var e=document.getElementById(this.id);for(var i=0;i<this.queue.length;i++){if(this.queue[i]==id){this.queue[i]="";}}
...
this.create();});Dialog.prototype.getId=function(title){return this.id;};Dialog.prototype.create=function(content){};Dialog.prototype.setTitle=function(title){this.title=title;DialogManager.show();};Dialog.prototype.open=function(arg){if(this.block==true){Mask.show(this.id);}
return this.show(arg);};Dialog.prototype.close=function(){this.setVisiable(false,false);if(this.parent!=null){this.parent.childs.remove(this);}
this.childs.each(function(e){e.close();});};Dialog.prototype.exit=function(){this.destroy();};Dialog.prototype.destroy=function(){DialogManager.pop(this);var c=this.getContainer();if(c!=null){c.parentNode.removeChild(c);}
...
catch(e){}}};Dialog.prototype.setActive=function(active){DialogManager.setActive(this,active);};Dialog.prototype.setActiveStyle=function(active){var c=this.getContainer();if(c!=null){if(active==true){c.style.borderColor="#999999";c.style.zIndex=DialogManager.getZIndex();}
else{c.style.borderColor="#dddddd";c.style.zIndex=this.zIndex;}}};Dialog.prototype.show=function(arg){var c=this.getContainer();if(c!=null){c.style.display="block";var width=this.getWidth(c);var height=this.getHeight(c);var top=document.documentElement.scrollTop+document.body.scrollTop;var left=document.documentElement.scrollLeft+document.body.scrollLeft;var y=top+parseInt((document.documentElement.clientHeight-height)/2);var x=left+parseInt((document.documentElement.clientWidth-width)/2);c.style.top=(y>0?y:0)+"px";c.style.left=(x>0?x:0)+"px";this.setVisiable(true,true);}
...
buffer[buffer.length]="</ul>";return buffer.join("");};var DialogUtil={};DialogUtil.messageTitle="Alter";DialogUtil.confirmTitle="Ensure";DialogUtil.ensureButtonText="Ok";DialogUtil.cancelButtonText="Ensure";DialogUtil.alert=function(message,callback){var messageDialog=new MessageDialog();messageDialog.setTitle(this.messageTitle);messageDialog.setEnsureText(this.ensureButtonText);messageDialog.ensure=callback;messageDialog.open(message);};DialogUtil.confirm=function(message,callback){var confirmDialog=new ConfirmDialog();confirmDialog.setTitle(this.messageTitle);confirmDialog.setEnsureText(this.ensureButtonText);confirmDialog.setCancelText(this.cancelButtonText);confirmDialog.ensure=callback;confirmDialog.cancel=callback;confirmDialog.open(message);};window.com=com;window.KeyCode=KeyCode;window.EventUtil=EventUtil;window.Dialog=Dialog;window.DialogManager=DialogManager;window.TaskManager=TaskManager;window.MessageDialog=MessageDialog;window.ConfirmDialog=ConfirmDialog;window.ContextMenu=ContextMenu;window.Dragable=Dragable;window.DialogUtil=DialogUtil;})();