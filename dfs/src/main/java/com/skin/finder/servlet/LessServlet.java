...
public class LessServlet extends BaseServlet {
    ...
    public void getRange(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(!allow(request, response, file)) {
            return;
        }
    ...
    private void getRange(HttpServletRequest request, HttpServletResponse response, File file) throws ServletException, IOException {
        ...
        try {
            ...
            if(type == 1) {
                ...
                Less.success(response, raf, charset, range);
                return;
            }
            else {
            ...
            Less.error(response, charset, 500, "error");
            return;
        }
        finally {
            IO.close(raf);
     ...
     * @throws IOException
     */
    public static boolean allow(HttpServletRequest request, HttpServletResponse response, File file) throws IOException {
        String allows = ConfigFactory.getTextType();
        String extension = FileType.getExtension(file.getName()).toLowerCase();
        ...
        String script = "(function(){window.location.href=\"?action=tail.error\";})()";
        Response.write(request, response, "text/javascript; charset=utf-8", script);
        return false;
    }
}
